...
Value.prototype.toBytes = function() {
  ...
  case ColumnType.VT_INTEGER:
    buffs.push(type);
    var integer = Buffer.alloc(8);
    var val = Long.fromNumber(this.value);
    integer.writeInt32LE(val.low, 0);
    integer.writeInt32LE(val.high, 4);
    buffs.push(integer);
    break;
  case ColumnType.VT_DOUBLE:
    buffs.push(type);
    var double = Buffer.alloc(Header.LITTLE_ENDIAN_64_SIZE);
    double.writeDoubleLE(this.value, 0);
    buffs.push(double);
    break;
