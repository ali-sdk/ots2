...
class Row {
  ...
  toBytes() {
    ...
    if (typeof this.deleteMarker !== 'undefined') {
      buffs.push(new Buffer([Tag.DELETE_MARKER]));
      del = 0x01;
    }
    ...
    buffs.push(new Buffer([Tag.ROW_CHECKSUM, checksum]));
...
Value.prototype.toBytes = function() {
  ...
  case ColumnType.VT_DOUBLE:
    buffs.push(type);
    var buf = new Buffer(Header.LITTLE_ENDIAN_64_SIZE);
    buf.writeDoubleLE(this.value, 0);
    buffs.push(buf);
    break;
  case ColumnType.VT_BOOLEAN:
    buffs.push(type);
    buffs.push(new Buffer([(this.value ? 1 : 0)]));
    break;
  case ColumnType.VT_STRING:
    var valueLength = new Buffer(Header.LITTLE_ENDIAN_32_SIZE);
    var value = new Buffer('' + this.value);
  ...
  case ColumnType.VT_BLOB:
    buffs.push(type);
    var buf = new Buffer(Header.LITTLE_ENDIAN_32_SIZE);
...
class Cell {
  ...
  toBytes() {
    ...
    // tag_cell_name
    buffs.push(new Buffer([Tag.CELL_NAME]));
    ...
    if (typeof this.value !== 'undefined' ||
      ...
      // tag_cell_value
      buffs.push(new Buffer([Tag.CELL_VALUE]));
      // formated_value
    ...
    if (this.op) {
      // tag_cell_op
      buffs.push(new Buffer([Tag.CELL_OP_TYPE]));
      // cell_op_value
      buffs.push(new Value(this.op).toBytes());
      // TODO
      // delete_all_version = 0x01 (1byte)
      // delete_one_version = 0x03 (1byte)
      // crc = crc8Int64(crc, cellTs);
    }
    ...
    if (this.ts) {
      ...
      // tag_cell_ts
      buff.writeUInt8(Tag.CELL_TS);
      // cell_ts_value
      buff.writeUInt64LE(this.ts);
      buffs.push(buff);

      // crc = crc8Int8(crc, cellOpType);
    }

    // cell_checksum = tag_cell_checksum row_crc8
    buffs.push(new Buffer([Tag.CELL_CHECKSUM, checksum]));
...
var createRow = function(primaryKeys, columns, deleteMarker) {
  ...
  if (primaryKeys && Object.keys(primaryKeys).length > 0) {
    let cells = exports.$(primaryKeys).map(function(key) {
      return new Cell(key);
    });
    pk = new PK(cells);
  }
  ...
  if (columns && Object.keys(columns).length > 0) {
    let cells = exports.$(columns).map(function(key) {
      return new Cell(key);
    });
    attr = new Attr(cells);
  }
